#!/usr/bin/env node
// Repository invariant: this plugin is English-only and ASCII-only, with exactly one documented
// exception - the SENSITIVE_PATTERNS regexes in lib/index.js, which must recognise Chinese
// sensitive content. Those CJK characters are functional, not prose, and they are the reason the
// plugin's own output can stay readable on the legacy consoles it warns about.
//
...
if (violations.length) {
  ...
  console.error("\nAllowed: CJK inside SENSITIVE_PATTERNS (lib/index.js). Everything else must be plain ASCII English.");
  process.exit(1);
}
...
console.log(`Allowed exception: ${allowed} CJK character(s) inside SENSITIVE_PATTERNS (lib/index.js lines ${range.start}-${range.end}).`);
