...
// Linux/macOS best effort: zenity, then kdialog, then osascript. The shell performs the
// redirection so the result never has to travel through a stdout pipe.
function posixReportDialogScript() {
  return [
    "#!/bin/sh",
    'REPORT="$1"; MANIFEST="$2"; RESULT="$3"; MAILTO="$4"; SUBJECT="$5"',
    'TEXT="A file format report was prepared on this computer. It lists only file types this plugin could not identify: extensions, magic byte prefixes, counts and format relationships. No file names, no folder paths and no file contents are included. Nothing is sent automatically - the choice is yours."',
    'cleanup() { rm -f "$RESULT" 2>/dev/null; }',
    'write_result() { printf \'{"action":"%s"}\\n\' "$1" > "$RESULT"; }',
    "if command -v zenity >/dev/null 2>&1; then",
    '  SEL=$(zenity --list --title="mega-index-map - file format report" --text="$TEXT" --column=Pick --column=Action TRUE "Send by e-mail" FALSE "Open folder" FALSE "Delete report" FALSE "Close" --height=420 --width=680 2>/dev/null)',
    '  case "$SEL" in',
    '    "Send by e-mail") write_result email ;;',
    '    "Open folder") command -v xdg-open >/dev/null 2>&1 && xdg-open "$(dirname "$REPORT")"; write_result folder ;;',
    '    "Delete report") rm -f "$REPORT"; write_result delete ;;',
    '    *) write_result close ;;',
    "  esac",
    "elif command -v kdialog >/dev/null 2>&1; then",
    '  SEL=$(kdialog --title "mega-index-map - file format report" --menu "$TEXT" 1 "Send by e-mail" 2 "Open folder" 3 "Delete report" 4 "Close" 2>/dev/null)',
    '  case "$SEL" in',
    '    1) write_result email ;;',
    '    2) command -v xdg-open >/dev/null 2>&1 && xdg-open "$(dirname "$REPORT")"; write_result folder ;;',
    '    3) rm -f "$REPORT"; write_result delete ;;',
    '    *) write_result close ;;',
    "  esac",
    "elif command -v osascript >/dev/null 2>&1; then",
    '  BTN=$(osascript -e \'button returned of (display dialog "A file format report was prepared on this computer. Nothing is sent automatically - the choice is yours." with title "mega-index-map - file format report" buttons {"Send by e-mail", "Open folder", "Delete report", "Close"} default button 1)\' 2>/dev/null)',
    '  case "$BTN" in',
    '    "Send by e-mail") open "mailto:$MAILTO?subject=$SUBJECT"; write_result email ;;',
    '    "Open folder") open "$(dirname "$REPORT")"; write_result folder ;;',
    '    "Delete report") rm -f "$REPORT"; write_result delete ;;',
    '    *) write_result close ;;',
    "  esac",
    "else",
...
async function runReportDialog(reportPath, opts = {}) {
  ...
  if (process.platform === "win32") {
    ...
    command = "sh";
    args = [scriptPath, reportPath, manifestPath, resultPath, REPORT_MAIL_TO, subject];
  }
  ...
  return { action: result.action || "closed", savedTo: result.savedTo || null, gui: true, at: result.at || null };
}
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      if (op === "deliver") {
        ...
        const done = ["email", "save", "delete", "close", "selftest"].includes(d.action);
        return {
