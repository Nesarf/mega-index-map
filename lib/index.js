...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute() {
      ...
      // version is the one that survives - keeping the oldest would silently roll a record back.
      ...
      return { count: dedup.length, store: indexFile(), conflicts, superseded: superseded.length };
    },
  }));
