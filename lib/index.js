...
function adbPath() {
  return resolveTool(
    "ADB_PATH",
    ["%TOOLCHAIN_HOME%\\\\adb.exe", "/usr/bin/adb", "/usr/local/bin/adb", "/opt/homebrew/bin/adb", "/opt/local/bin/adb", "/snap/bin/adb"],
    "adb"
  );
}
...
// The primary `path` entries below describe ONE machine's layout (the maintainer's), which is
// what this plugin was built to catalogue. A path that does not exist is simply skipped by
// resolveCandidatePath(), so on any other machine these entries resolve to nothing and cost
// nothing; use `library_detect op=propose` plus `op=add` to seed a different layout, and set
// FFPROBE_PATH / MEDIAINFO_PATH / ADB_PATH to override the media and device engines.
const DETECT_CANDIDATES = [
  { type: "tool", name: "MediaInfo", path: "%TOOLCHAIN_HOME%\\\\downloads\\mediainfo-cli\\MediaInfo.exe", alts: ["/usr/bin/mediainfo", "/usr/local/bin/mediainfo", "/opt/homebrew/bin/mediainfo", "/opt/local/bin/mediainfo", "/snap/bin/mediainfo"], desc: "media metadata analysis" },
  { type: "tool", name: "ffmpeg", path: "%TOOLCHAIN_HOME%\\\\bin\\ffmpeg.exe", alts: ["/usr/bin/ffmpeg", "/usr/local/bin/ffmpeg", "/opt/homebrew/bin/ffmpeg", "/opt/local/bin/ffmpeg", "/snap/bin/ffmpeg"], desc: "transcoding / media processing" },
  { type: "tool", name: "ffprobe", path: "%TOOLCHAIN_HOME%\\\\bin\\ffprobe.exe", alts: ["/usr/bin/ffprobe", "/usr/local/bin/ffprobe", "/opt/homebrew/bin/ffprobe", "/opt/local/bin/ffprobe", "/snap/bin/ffprobe"], desc: "media probing (alternative fingerprint source)" },
  { type: "tool", name: "uv", path: "%TOOLCHAIN_HOME%\\\\uv\\uv.exe", alts: ["/usr/local/bin/uv", "/opt/homebrew/bin/uv", "/opt/local/bin/uv", "/snap/bin/uv", "~/.local/bin/uv"], desc: "Python package/interpreter manager" },
  ...
  { type: "tool", name: "Go", path: "%TOOLCHAIN_HOME%\\\\go\\bin\\go.exe", alts: ["/usr/local/go/bin/go", "/opt/homebrew/bin/go", "/opt/local/bin/go", "/snap/bin/go"], desc: "Go 1.27.1 toolchain (GOPATH/GOCACHE/GOMODCACHE on a second drive)" },
  { type: "tool", name: "Git", path: "C:\\Program Files\\Git\\cmd\\git.exe", alts: ["/usr/bin/git", "/usr/local/bin/git", "/opt/homebrew/bin/git", "/opt/local/bin/git", "/snap/bin/git"], desc: "Git 2.55.0 for Windows" },
...
async function runReportDialog(reportPath, opts = {}) {
  ...
  if (!result) {
    if (launchError) {
      ...
      return { action: "unavailable", gui: false, note: `the dialog program could not be started (${why}); nothing was sent` };
    }
    return { action: child.exitCode === null ? "pending" : "unavailable", gui: false, note: "the dialog did not report back; no report was sent" };
  }
  const action = result.action || "closed";
  // "unavailable" means the script found no desktop toolkit: that host has no GUI to show.
  return { action, savedTo: result.savedTo || null, gui: action !== "unavailable", at: result.at || null };
}
...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    name: "library_index",
    description:
      "Rebuild/dedupe the Library index (by type+name+source), sort by createdAt, and rewrite index.json. When a key holds several versions the newest one survives and the dropped ones are reported (and kept in the append-only log), so a rebuild never rolls a record back. Returns count, store path, conflicts and how many versions were superseded.",
    parameters: {},
    output: {
      schema: { type: "object", additionalProperties: false, properties: { count: { type: "number" }, store: { type: "string" }, superseded: { type: "number" }, conflicts: { type: "array", items: { type: "object", additionalProperties: true } } } },
    ...
    async execute() {
      ...
      return { count, store: indexFile(), conflicts, superseded: superseded.length };
    },
  }));
    ...
    async execute(args) {
      ...
      if (op === "deliver") {
        ...
        return {
          ...
          op, file, action: d.action, savedTo: d.savedTo, gui: d.gui,
      ...
      if (op === "add") {
        ...
        if (args.textExtension) {
          ...
          next.nameRules.push({ re, source: String(args.nameRule), role: String(args.role || "local"), expect: args.expect || null, note: String(args.note || "").slice(0, 200) });
