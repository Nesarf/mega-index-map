...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    name: "library_format",
    description:
      "Inspect and extend this installation's format library. op=list shows the local pack and the last report date; op=scan sweeps a directory and proposes file types the library cannot identify yet (grouped by extension with evidence); op=learn registers a magic signature intersected from explicit sample files; op=add registers a signature, a plain-text extension, or a name rule by hand; op=remove drops a local entry; op=report writes a small, inspectable contribution archive (extensions, magic prefixes, counts — no file names, no paths, no contents) that the USER may choose to send. Built-ins are always matched first and conflicts are refused unless confirm:true, so a local entry can never shadow a known format. Scanning alone never learns anything, and this tool performs no network I/O — reports are delivered manually by the user.",
    parameters: {
      op: { type: "string", required: true, description: "list | scan | learn | add | remove | report" },
      ...
      out: { type: "string", description: "report: output .zip path; defaults to $DSH_HOME/library/reports/format-report-<timestamp>.zip" },
    ...
    async execute(args) {
      ...
      if (op === "report") {
        if (!args.dir) return { ok: false, op, note: "report needs dir (the directory to summarise)", scanned: 0, unidentified: [] };
        const { scanned, groups } = collectUnidentified(args.dir, args.maxFiles || 1500, 6, false);
        const unidentified = [...groups.values()].sort((a, b) => b.count - a.count).slice(0, 200).map((g) => {
          const prefix = commonPrefix(g.heads);
          const suggestion = prefix.slice(0, 8);
          return {
        ...
        const report = {
          ...
          privacy: "Collected: file extensions, magic-byte prefixes, counts, plugin version, local pack entries (no notes). NOT collected: file names, directory paths, file contents, user name, host name, credentials.",
        ...
        const manifest = [
          ...
          "  report.json  — machine-readable summary (extensions, magic prefixes, counts)",
          "  manifest.txt — this human-readable summary",
          "",
          "NOT included, by design:",
          "  file names, directory paths, file contents, user name, host name, credentials.",
          "",
        ...
        const buf = zipBuffer([{ name: "report.json", data: reportJson }, { name: "manifest.txt", data: "\uFEFF" + manifest }]);
        ...
        const obj = {
          ...
          source: "local",
          path: outFile,
          description: `Format-library contribution archive: ${unidentified.length} unidentified type(s) out of ${scanned} scanned file(s); extensions and magic prefixes only (no file names, paths or contents); built locally, not sent`,
        ...
        saveIndex(index);
        appendLog({ op: "format-report", type: "format", name: null, source: "local", disposition: null, detail: `report written locally: ${unidentified.length} unidentified type(s) from ${scanned} file(s), ${buf.length} bytes (not sent)` });
        return {
          ok: true, op, file: outFile, bytes: buf.length, scanned,
          ...
          note: `report written locally (${buf.length} bytes, ${unidentified.length} unidentified type(s)) — nothing was uploaded; review manifest.txt, then send it yourself if you choose to`,
        };
      }
      ...
      return { ok: false, op, note: `unknown op "${args.op}", allowed: list, scan, learn, add, remove, report` };
    },
  }));
